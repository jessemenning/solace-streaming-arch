...
Fleet Agent UI automatically. The fleet generator and AI agent UI start automatically
as containers — no host Python required.
...
(SMF protocol). Two SOURCEs map to two queues — each SOURCE receives connector metadata
columns from the Solace message envelope: `_rw_solace_destination` (topic) and
`_rw_solace_timestamp` (sender time). Routing MVs use
`WHERE _rw_solace_destination LIKE '...'` to replace Solace wildcard subscriptions.

Two queues (and two SOURCEs) prevent alert messages from being blocked behind high-volume telemetry:
- `fleet_ingest_telemetry` SOURCE → `rw-ingest` queue — `fleet/telemetry/>` + `fleet/commands/>`
...
├── config/
│   ├── solace/
│   │   └── setup.sh                  SEMP v2 — VPN, queues rw-ingest + events-ingest
│   ├── risingwave/
