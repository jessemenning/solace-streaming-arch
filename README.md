...
| Delivery | Python proxy (`solace-proxy`) | Subscribes to queues, injects topic + timestamp as HTTP headers, POSTs to RisingWave webhook |
...
The script builds images for all services; starts the stack; configures
Solace Platform (VPN, queues, subscriptions); initializes RisingWave; and opens the
Fleet Agent UI automatically. The fleet generator, Python proxy, and AI agent UI start
automatically as containers — no host Python required.
...
Messages flow through two Solace queues into the Python proxy (`solace-proxy`). The proxy
HTTP POSTs each payload to RisingWave's webhook endpoint, injecting the Solace topic and
sender timestamp as HTTP headers (`x-message-topic`, `x-message-timestamp`). RisingWave
captures them via `INCLUDE header` as first-class VARCHAR columns (`_topic`, `_timestamp`).
Routing MVs use `WHERE _topic LIKE '...'` to replace Solace wildcard subscriptions.
...
├── config/
│   ├── solace/
│   │   └── setup.sh                  SEMP v2 — VPN, queues rw-ingest + events-ingest (no RDP — proxy binds directly)
│   ├── risingwave/
...
├── proxy/
│   ├── proxy.py                      Solace queue consumer → RisingWave webhook proxy (injects topic + timestamp headers)
│   ├── entrypoint.sh                 Waits for VPN + queues before starting
│   ├── Dockerfile
│   └── requirements.txt
├── generator/
...
1. Check proxy logs: `docker logs solace-proxy` — look for `delivered=N` stats and any HTTP errors  
2. Confirm queues exist: check SEMP at http://localhost:8180 → `streaming-poc` VPN → Queues → `rw-ingest`  
3. Re-run `config/solace/setup.sh` if queues are missing, then restart the proxy: `docker compose restart solace-proxy`
